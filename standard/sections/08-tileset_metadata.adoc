...
Imagine a case where a tileset covers the whole bounding box of a tile matrix set. Now,
...
tile column indices. This invalidates any cached tiles that the client could have
...
data. The area outside these limits is considered empty space and is not covered by the
tileset.
